...
from dtos.general_dto import ResponseSchema, Role
from services.auth.user_service import UserService
from utils.auth import create_access_token, role_required
...
async def get_all(
    rol: int,
    user_roles: List[Role] = Depends(
        role_required([Role.user, Role.admin])
    ),
):
    try:
        # Obtener todos los usuarios
        data: List[UserDTO] = await UserService.get_all(rol)
